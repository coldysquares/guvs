...
Confirm `dist/` contains the root registry, the explicit `shared/` membrane runtime, and exactly eight registered app directories: POND Graf, PSR, AWD, Router, Aster Graf, Fungi Cell Map, Wiki Constellation, and Saperli Popette. `substrate-001/`, app-local `api/`, `.vercel`, backup, and deployment-config files must not appear in the static artifact.
...
- `registry.json` contains all eight GUVs and excludes Substrate 001.
- `/psr/`, `/awd/`, `/router/`, `/aster-graf/`, `/fungi-cell-map/`, `/wiki-constellation/`, `/pond-graf/`, `/pond-graf/wiki/`, and `/saperli-popette/` serve the correct applications.
...
- The existing Aster Graf and Wiki Constellation routes still serve their V2 interfaces.
- POND Graf opens Family and Pitch through the same membrane runtime; entering, Back, and Pond exit preserve state on mobile.
- The POND Wiki lens accumulates a second fetched ring without replacing the first and returns to the exact prior membrane.
