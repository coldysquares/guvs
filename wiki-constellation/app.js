...
function renderSelection() {
  if (!selected) {
    els.selectionKind.textContent = "Selected star";
    els.selectionTitle.textContent = "Choose a star";
    els.selectionNote.textContent = "Tap any star, or use the linked-article index below.";
    els.selectionLink.hidden = true;
  } else {
    els.selectionKind.textContent = selected.kind === "origin" ? "Origin article" : "Linked article";
    els.selectionTitle.textContent = selected.name;
...
function selectNode(node, shouldFly = true) {
  selected = node;
  renderSelection();
  if (node && shouldFly) flyToNode(node);
}
...
els.canvas.addEventListener("pointermove", (event) => {
  const point = pointerPosition(event);
  if (pointerStart && Math.hypot(point.x - pointerStart.x, point.y - pointerStart.y) > 5) moved = true;
...
els.freeze.addEventListener("click", () => {
  ...
  els.freeze.setAttribute("aria-pressed", String(physicsFrozen));
  els.freeze.textContent = physicsFrozen ? "Unfreeze" : "Freeze";
});

els.linkFilter.addEventListener("input", renderLinkList);

async function loadQuery(rawQuery, { updateHistory = true } = {}) {
